...
Dosya: swing_bot.py
Tarih: 12 Şubat 2026
Açıklama: ccxt ile borsa bağlantısı, pandas_ta ile teknik indikatörler kullanarak
          swing trade sinyalleri üreten ve uygulayan ana bot modülü.
"""
import ccxt
import pandas as pd
...
SCAN_COIN_COUNT = 100          # İLK 100 COİN
...
USE_TOP_PERFORMERS = False  # False = TÜM COİNLERİ TARA (100 ADET)
...
def analyze_timeframe_dual(symbol, timeframe, btc_trend):
    ...
    try:
        ...
        # Canlı İzleme Çıktısı (Anlık Veri)
        if timeframe == '15m':
             status_icon = "⚪"
             if long_score > 30: status_icon = "🟢"
             elif short_score > 30: status_icon = "🔴"
             
             print(f"   ⏱️ 15dk Analiz: Fiyat ${price:.5f} | RSI: {rsi:.1f} | ADX: {adx:.1f} | Skor: L{long_score}/S{short_score} {status_icon}")
...
def analyze_coin_swing(symbol, rank, btc_trend):
    ...
    try:
        ...
        for tf in TIMEFRAMES:
            ...
            time.sleep(0.01)  # Kapanış hızı max (0.2 -> 0.01)
...
def get_top_coins(limit=SCAN_COIN_COUNT):
    """24 Saatlik Yükselişe (Yüzde Değişime) Göre Top Coinleri Al."""
    try:
        # Static listeyi kullanmıyoruz, daima güncel veri.
        tickers = exchange.fetch_tickers()
        
        futures = []
        for symbol, data in tickers.items():
            # Sadece USDT Futures çiftleri (Örn: BTC/USDT:USDT)
            if ':USDT' in symbol and data.get('quoteVolume'):
                vol = float(data.get('quoteVolume', 0))
                
                # Hacim Filtresi: Min Hacim Altındakileri Alma
                if vol >= MIN_VOLUME_USD:
                    pct = float(data.get('percentage', 0) or 0) # 24s Değişim Yüzdesi
                    futures.append({
                        'symbol': symbol,
                        'volume': vol,
                        'percentage': pct
        ...
        # 24 SAATLİK YÜKSELİŞE GÖRE SIRALA (En çok yükselen en üstte)
        # Kullanıcı talebi: "hacime göre değil, yükselişe göre sırala"
        futures.sort(key=lambda x: x['percentage'], reverse=True)
        
        top_coins = futures[:limit]
        
        # Bilgi mesajı
        print(f"📊 Market Analizi: {len(futures)} coin tarandı.")
        print(f"   En çok yükselen: {top_coins[0]['symbol']} (+%{top_coins[0]['percentage']:.2f})")
...
def run_swing_scanner():
    ...
    # Taranacak coinlerin listesini göster
    print("\n📋 TARANACAK COİNLER (24s Yükselişe Göre Sıralı):")
    coin_list_str = ", ".join([f"{c['symbol'].split('/')[0]}(%{c['percentage']:.1f})" for c in coins])
    print(f"{coin_list_str}")
    print("="*50 + "\n")
    ...
    for i, coin_data in enumerate(coins, 1):
        ...
        # Sürekli print yerine aynı satırda güncelle (Hız için)
        print(f"\r🚀 [{i}/{len(coins)}] {coin_name} (Dev: %{coin_data['percentage']:.1f}) Analiz ediliyor...", end="")
        ...
        try:
            ...
            if result:
                ...
                # EĞER SKOR YÜKSEKSE BEKLEME - DİREKT BAS
                if result['score'] >= 75:
                    dir_emoji = "🟢" if result['direction'] == 'LONG' else "🔴"
                    print(f"\n\n{'='*40}")
                    print(f"🔥 ACİL FIRSAT: {coin_name} {dir_emoji} {result['direction']}")
                    print(f"{'='*40}")
                    print(f"   ⭐ SKOR: {result['score']} | WR: %{result['win_rate']:.0f}")
                    print(f"   ⚡ Giriş: ${result['price']:.4f}")
                    print(f"   🎯 Hedef 1: ${result['tp1']:.4f}")
                    print(f"   🛑 Stop: ${result['stop_loss']:.4f}")
                    print(f"   📢 Telegram'a gönderiliyor...")
                    
                    msg = format_swing_signal(result)
        ...
        # time.sleep kaldırıldı - Max Hız
    
    # Döngü bitince satır atla
    print()
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            print()
            print(f"⏳ Sonraki tarama: 1 dakika içinde...")
            
            # 1 dakika bekle (Anlık hissi için kısaltıldı)
            for i in range(60, 0, -1):
                if i % 10 == 0:
                    print(f"\r♻️ Piyasalar güncelleniyor... {i}sn", end="")
                time.sleep(1)
            print("\r🚀 Yeni tarama başlıyor!           ")
