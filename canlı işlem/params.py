...
# ║          Bu dosyayı değiştir → bot yeniden başlatıldığında          ║
# ║          veya backtest çalıştırıldığında otomatik güncellenir.      ║
# ╚══════════════════════════════════════════════════════════════════════╝
#
# ❗ Değişiklik sonrası:
#    • Backtest  → hemen geçerli (her çalıştırmada yeniden yüklenir)
#    • Canlı bot → botu yeniden başlat (Ctrl+C → tekrar başlat)
#
...
# İLK Stop-Loss: Giriş fiyatının kaç % üstü? (SHORT için zarar = fiyat yukarı giderse)
# entry × (1 + SL_ABOVE_ENTRY_PCT/100)
# Örnek: entry=1.00, SL=%15 → SL=1.15 → maksimum %15 yukarı gidebilir
...
# Artık 0$ zararda kapanır (BE = sıfır zarar)
# Öneri: TSL_ACTIVATION_DROP_PCT ile aynı değer

TSL_ACTIVATION_DROP_PCT = 7.0
# Stage 2 — TSL Aktivasyonu: Fiyat %X aşağı inince Trailing Stop devreye girer
# Bu nokta ve sonrası SL otomatik olarak fiyatı takip eder
# Düşürürsen: TSL daha erken aktif (daha az kâr garantisi, ama daha güvenli)
# Artırırsan: TSL geç aktif (daha derin düşüş gerekir)

TSL_TRAIL_PCT = 4.0
# TSL mesafesi: SL = lowest_low × (1 + TSL_TRAIL_PCT/100)
# Örnek: lowest_low=0.80, TSL=%4 → SL=0.832
# Dar trailing: 2.0 → SL daha sıkı, daha erken kilitler
# Geniş trailing: 6.0 → SL daha uzak, daha fazla salınım tolere eder
...
# Test için düşük tut: 1 veya 2

MAX_ACTIVE_TRADES = 5
# Aynı anda maksimum açık pozisyon sayısı
# Test için: 1 veya 2 (sinyalleri tek tek takip etmek için)
...
# Test için: 60 (1 dakika) — API limit aşmamak için çok küçük yapma

WATCHLIST_CHECK_INTERVAL_SEC = 60
# Watchlist sinyal kontrol aralığı (saniye). Watchlist'teki coinler kaç saniyede kontrol edilir?
# Varsayılan: 60 saniye

MANAGER_INTERVAL_SEC = 5
# Açık trade yönetim döngüsü (saniye). SL, TSL, BE kontrolleri kaç saniyede bir?
# Varsayılan: 5 saniye — daha küçük yapma (API rate limit)
...
BACKTEST_SYMBOLS = [
    ...
    # Buraya test etmek istediğin coinleri ekleyebilirsin
